...
class Solution {
  public:
    void func(int ind,int sum,int n,vector<int>& arr,vector<int>& subset){
        if(ind==n){
            subset.push_back(sum);
            return;
        }
        //pick
        func(ind+1,sum+arr[ind],n,arr,subset);
        //not pick
        func(ind+1,sum,n,arr,subset);
    }
    vector<int> subsetSums(vector<int>& arr) {
        // code here
        vector<int> subset;
        func(0,0,arr.size(),arr,subset);
        return subset;
    }
};
